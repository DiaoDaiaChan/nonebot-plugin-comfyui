...
comfyui_parser.add_argument("prompt", nargs="*", help="正面提示词", type=str)
comfyui_parser.add_argument("-u", "-负面",nargs="*", dest="negative_prompt", type=str, help="负面提示词")
comfyui_parser.add_argument("-ar", "画面比例", dest="accept_ratio", type=str, help="画幅比例")
comfyui_parser.add_argument("-s", "-种子", dest="seed", type=int, help="种子")
comfyui_parser.add_argument("-t", "-步数", dest="steps", type=int, help="迭代步数")
comfyui_parser.add_argument("-cfg", dest="cfg_scale", type=float, help="CFG scale")
comfyui_parser.add_argument("-n", "-去噪", dest="denoise_strength", type=float, help="降噪强度")
comfyui_parser.add_argument("-高", "--height", dest="height", type=int, help="图片的高度")
comfyui_parser.add_argument("-宽", "--width", dest="width", type=int, help="图片的宽度")
comfyui_parser.add_argument("-o", dest="override", action="store_true", help="不使用预设的正面提示词")
comfyui_parser.add_argument("-on", dest="override_ng", action="store_true", help="不使用预设的负面提示词")
comfyui_parser.add_argument("-wf", "-工作流", dest="work_flows", type=str, help="选择工作流")
comfyui_parser.add_argument("-sp", "-采样器", dest="sampler", type=str, help="采样器")
comfyui_parser.add_argument("-sch", "-调度器", dest="scheduler", type=str, help="调度器")
comfyui_parser.add_argument("-b", "-数量", dest="batch_size", type=int, help="每批数量", default=1)
comfyui_parser.add_argument("-bc", "-批数", dest="batch_count", type=int, help="批数", default=1)
comfyui_parser.add_argument("-m", "-模型", dest="model", type=str, help="模型")
comfyui_parser.add_argument("-be", "-后端", dest="backend", type=str, help="后端索引或者url")
comfyui_parser.add_argument("-f", "-转发", dest="forward", action="store_true", help="使用转发消息")
...
comfyui_parser.add_argument("-r", "-shape", "-分辨率", dest="shape", type=str, help="自定义的比例字符串")
comfyui_parser.add_argument("-sil", "-静默", dest="silent", action="store_true", help="不返回各种提示消息")
comfyui_parser.add_argument("-notice", "-通知", dest="notice", action="store_true", help="工作流执行完成的时候私聊通知, 适用于长工作流")
...
async def rebuild_parser(wf, reg_args: dict | None = None):
    ...
    comfyui_parser.add_argument("-u", "-负面",nargs="*", dest="negative_prompt", type=str, help="Negative prompt")
    comfyui_parser.add_argument("-ar", "画面比例", dest="accept_ratio", type=str, help="Accept ratio")
    comfyui_parser.add_argument("-s", "-种子", dest="seed", type=int, help="Seed")
    comfyui_parser.add_argument("-t", "-步数", dest="steps", type=int, help="Steps")
    comfyui_parser.add_argument("-cfg", dest="cfg_scale", type=float, help="CFG scale")
    comfyui_parser.add_argument("-n", "-去噪", dest="denoise_strength", type=float, help="Denoise strength")
    ...
    comfyui_parser.add_argument("-on", dest="override_ng", action="store_true", help="不使用预设的负面提示词")
    comfyui_parser.add_argument("-wf", "-工作流", dest="work_flows", type=str, help="Workflows")
    comfyui_parser.add_argument("-sp", "-采样器", dest="sampler", type=str, help="采样器")
    comfyui_parser.add_argument("-sch", "-调度器", dest="scheduler", type=str, help="调度器")
    comfyui_parser.add_argument("-b", "-数量", dest="batch_size", type=int, help="每批数量", default=1)
    comfyui_parser.add_argument("-bc", "-批数", dest="batch_count", type=int, help="批数", default=1)
